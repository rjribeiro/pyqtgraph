...
def runStage(i):
    ...
    """
    with pg.ProgressDialog(f"Running stage {i}..", maximum=100, nested=True) as dlg:
        for _ in range(100):
            time.sleep(0.02)
            dlg += 1
            if dlg.wasCanceled():
                print(f"Canceled stage {i}")
                break
...
def runManyStages(i):
    ...
    """
    with pg.ProgressDialog(f"Running stage {i}..", maximum=3, nested=True, wait=0) as dlg:
        for j in range(1,4):
            runStage('%d.%d' % (i, j))
            dlg += 1
            if dlg.wasCanceled():
