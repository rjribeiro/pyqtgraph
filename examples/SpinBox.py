...
spins = [
    ...
    (
        ...
        pg.SpinBox(
            value=10, int=True, dec=True, minStep=1, step=1, decimals=4
        ),
    ),
    (
    ...
    (
        ...
        pg.SpinBox(
            ...
            evalFunc=lambda s: ast.literal_eval(f'0x{s}'),
        ),
    ),
    (
...
def valueChanged(sb):
    changedLabel.setText(f"Final value: {str(sb.value())}")

def valueChanging(sb, value):
    changingLabel.setText(f"Value changing: {str(sb.value())}")
