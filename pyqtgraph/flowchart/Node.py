...
class Node(QtCore.QObject):
    ...
    def dependentNodes(self):
        ...
        for t in self.inputs().values():
            nodes |= {i.node() for i in t.inputTerminals()}
        return nodes
    ...
    def inputValues(self):
        ...
        return {n: t.value() for n, t in self.inputs().items()}
            
    def outputValues(self):
        """Return a dict of all output values currently generated by this node."""
        return {n: t.value() for n, t in self.outputs().items()}
    ...
    def processBypassed(self, args):
        ...
        for term in list(self.outputs().values()):
            ...
            result[term.name()] = None if byp is None else args.get(byp, None)
        return result
    ...
    def restoreTerminals(self, state):
        ...
        for name, opts in state.items():
            ...
            try:
                ...
                printExc(f"Error restoring terminal {str(name)} ({str(opts)}):")
